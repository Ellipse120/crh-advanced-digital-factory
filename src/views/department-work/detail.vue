          ...
          <el-card shadow="hover">
            ...
            <el-table-wrapper :data="model.workContents" border stripe>
              ...
              <el-table-column align="center" label="责任人" prop="responseUserId">
                <template slot-scope="{ row }">
                  <user-selector
                    v-model="row.responseUserId"
                    class="w-full"
                    @selected-item="(val) => handleSelect(val, row)"
                  />
                </template>
              </el-table-column>
              <el-table-column align="center" label="操作" width="100">
                <template slot="header" slot-scope="{ row }">
                  <el-link type="primary" class="mr-10 font-12" @click="doAddWorkContent(row)"><i class="el-icon-plus" />添加
                  </el-link>
                </template>
                <template slot-scope="{ $index }">
                  <el-link type="danger" class="mr-10 font-12" @click="doRemoveWorkContent($index)"><i
...
const planType = mapper.DEPT[0]

export default {
  name: 'DepartmentWorkDetail',
  ...
  methods: {
    ...
    doAddWorkContent () {
      ...
      this.model.workContents.push(data)
    },
