<template>
  <div class="app-container page-content--user-management">
    ...
    >
      <el-table-column
        ...
        align="center"
        width="80"
        class-name="col--color-red cell--user-id"
      />
      <el-table-column
        label="登陆名"
        prop="loginName"
        align="center"
        width="200"
        class-name="col--color-orange cell--user-name"
      />
      <el-table-column
        label="用户名"
        prop="username"
        align="center"
        width="220"
        class-name="col--color-yellow cell--user-display-name"
      />
      <el-table-column
        label="部门"
        prop="departmentId"
        align="center"
        min-width="130"
        class-name="col--color-green cell--department"
      >
        <template slot-scope="{ row }">
          {{ row.departmentModel ? row.departmentModel.departmentName : row.departmentId }}
        </template>
      </el-table-column>
      <el-table-column
        prop="roleList"
        ...
        label="角色"
        min-width="220"
        class-name="col--color-cyan"
      >
        <template slot-scope="{ row }">
          {{ row.roleList ? row.roleList.map(o => o.roleName).toString() : '' }}
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        align="center"
        width="370"
        class-name="cell--operations"
      >
        <template slot-scope="scope">
          <el-button type="primary" @click="doSetRole(scope)">配置角色</el-button>
          <el-button type="success" @click="doEdit(scope)">编辑</el-button>
          <hr>
          <el-button type="warning" @click="doResetPassword(scope)">重置密码</el-button>
    ...
    >
      <el-form ref="form" :model="model" :rules="rules" label-width="120px">
        ...
        <el-form-item label="部门" prop="departmentId">
          <el-select v-model="model.departmentId" class="full-width" clearable filterable>
...
export default {
  ...
  methods: {
    ...
    async doSetRole (scope) {
      ...
      this.selectedRoles = userDetail?.data?.roleList.map(o => o.id)
